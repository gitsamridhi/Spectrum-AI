          ...
          return (
            <button key={ch.id} onClick={() => handleSelectChannel(ch.id)}
              className={`w-full flex items-center justify-between px-5 h-12 rounded-xl border text-left transition-all duration-200 cursor-pointer ${
                sel ? "border-zinc-900 bg-zinc-900" : "border-zinc-200 bg-white hover:border-zinc-300 hover:bg-zinc-50"
              }`}>
              <span className={`text-[13px] font-semibold ${sel ? "text-white" : "text-zinc-700"}`}>{ch.label}</span>
