...
function NavTooltip({
  label, children, T,
}: { label: string; children: React.ReactNode; T: ReturnType<typeof useTheme>['T'] }) {
  ...
  return (
    <div ref={ref} onMouseEnter={handleEnter} onMouseLeave={() => setHov(false)}>
      {children}
      {hov && rect && (
        <div
          className="fixed px-2.5 py-1.5 rounded-lg text-[11px] font-semibold whitespace-nowrap z-[9999] shadow-lg pointer-events-none"
          style={{
...
export default function Sidebar({
  ...
  const navItem = (icon: React.ElementType, label: string, id: string) => {
    ...
    if (collapsed) {
      return (
        <NavTooltip key={id} label={label} T={T}>
          <button
  ...
  return (
    <aside
      className="shrink-0 flex flex-col h-full select-none transition-all duration-250"
      style={{
      ...
      <div className={collapsed ? 'px-1.5 pt-4 pb-2' : 'px-3 pt-4 pb-3'}>
        <div className={`flex items-center ${collapsed ? 'justify-center mb-2' : 'gap-2 mb-3.5 px-1'}`}>
          <div className="w-4 h-4 shrink-0" style={{ color: T.text }}><Logo /></div>
          {!collapsed && (
      ...
      <div className={collapsed ? 'px-1.5 pb-2' : 'px-3 pb-2'}>
        {collapsed ? (
          <NavTooltip label="Create" T={T}>
            <button onClick={() => setActive('image')}
              className="w-full flex items-center justify-center p-2.5 rounded-xl cursor-pointer transition-colors"
      ...
      <div className={`${collapsed ? 'mx-1.5' : 'mx-3'} shrink-0`} style={{ height: 1, background: T.borderMuted }} />
      ...
      <div className="flex-1 overflow-y-auto min-h-0 mt-2" style={{ scrollbarWidth: 'none' }}>
        ...
        <div className={`${collapsed ? 'mx-1.5' : 'mx-3'} my-2`} style={{ height: 1, background: T.borderMuted }} />
        ...
        {/* Advanced Studios */}
        <div className={collapsed ? 'px-2 pt-2 pb-4' : 'px-3 pt-3 pb-4'}>
          {collapsed ? (
            <div className="space-y-0.5">
      ...
      {/* Bottom bar */}
      <div
        className={collapsed ? 'px-1.5 py-2 shrink-0' : 'px-3 py-3 shrink-0'}
        style={{ borderTop: `1px solid ${T.borderMuted}` }}>
        {collapsed ? (
          <div className="flex flex-col items-center gap-1.5">
            <NavTooltip label={isDark ? 'Light mode' : 'Dark mode'} T={T}>
            ...
            <NavTooltip label={displayName} T={T}>
              <div className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold cursor-default"
                style={{ background: T.text, color: T.bg }}>
                {avatar}
              </div>
            </NavTooltip>
        ...
        {/* Collapse toggle */}
        <div className={`flex ${collapsed ? 'justify-center' : 'justify-end'} mt-2`}>
          <button
